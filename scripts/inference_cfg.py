...
from ldm.models.diffusion.plms_cfg import PLMSSampler
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default = 0.8,
        help="model-based guidance scale",
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="path to pretrained config with constructs pretrained model",
    )
    parser.add_argument(
        "--ckpt",
        type=str,
    ...
    pretrained_config = OmegaConf.load(f"{opt.pretrained_config}")
    model = load_model_from_config(config, f"{opt.ckpt}")
    seed_everything(opt.seed)
    pretrained_model = load_model_from_config(pretrained_config, f"{opt.pretrained_ckpt}")
    ...
    if opt.plms:
        sampler = PLMSSampler(model, pretrained_model)
    else:
    ...
    with torch.no_grad():
        with precision_scope("cuda"):
            with model.ema_scope():
                ...
                mask=Image.open(opt.mask_path).convert("L").resize((512,512))
                ...
                f, c = model.get_learned_conditioning(ref_tensor.to(torch.float16))
               
                c = pretrained_model.proj_out(c)
                f = model.proj_out2(f)
                ...
                samples_ddim, _ = sampler.sample(S=opt.ddim_steps,
                                                    ...
                                                    model_guidance_scale=opt.model_cfg_scale,
                                                    unconditional_conditioning=uc,
                                                    eta=opt.ddim_eta,
                                                    x_T=start_code,
                                                    cfg_flag = opt.cfg_flag,
                                                    cfg_timestep = opt.cfg_timestep, 
                ...
                if not opt.skip_save:
                    for i,x_sample in enumerate(x_checked_image_torch):
                        ...
                        img.save(os.path.join(grid_path, 'grid-'+filename[:-4]+'_'+str(opt.seed)+str(opt.reference_path[-5]) +str(opt.model_cfg_scale) + str(opt.scale) + '.png'))
