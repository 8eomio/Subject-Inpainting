...
class SubjectImageDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_path=os.path.join('dataset/subject-images/images',file_name)
        mask_path = os.path.join('dataset/subject-images/mask',file_name)
        ...
        mask[mask < 0.5] = 0
        mask[mask >= 0.5] = 1
        maskGT_tensor = torch.from_numpy(mask)
        mask_reshaped = maskGT_tensor.squeeze().numpy()[:, :, None]
        '''
        ### Generate mask
        ...
        if W - bbox[2] < 0:
            print("invalid left_freespace")
            right_freespace = abs(W-bbox[2])
        else:
            right_freespace=W-bbox[2]
        ...
        if H-bbox[3] < 0:
            print("invalid down_freespace")
            down_freespace = abs(H- bbox[3])
            print(down_freespace)
        else:
            down_freespace=H-bbox[3]
        if left_freespace == 0:
            left_freespace = 1
        ...
        if W > H:
            ...
            if right_most<= left_most:
                ...
                free_space=abs(min(extended_bbox[1]-0,extended_bbox[0]-left_pos,left_pos+H-extended_bbox[2],H-extended_bbox[3]))
            ...
            if lower_most<=upper_most:
                ...
                free_space=abs(min(extended_bbox[1]-upper_pos,extended_bbox[0]-0,W-extended_bbox[2],upper_pos+W-extended_bbox[3]))
        ...
        #maskGT_tensor_resize = T.Resize([self.args['image_size'],self.args['image_size']])(maskGT_tensor)
