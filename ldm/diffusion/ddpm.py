...
class DDPM(pl.LightningModule):
    ...
    def get_input(self, batch, k):
        ...
        #maskGT = maskGT.to(memory_format=torch.contiguous_format).float()
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        self.proj_out2 = nn.Linear(1024,768)
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
                  cond_key=None, return_original_cond=False, bs=None,get_mask=False,get_reference=False,get_inpaint=False):
        ...
        if self.model.conditioning_key is not None:
            ...
            if not self.cond_stage_trainable or force_c_encode:
                if isinstance(xc, dict) or isinstance(xc, list):
                    ...
                    f, c = self.get_learned_conditioning(xc.to(self.device))
                    f = self.proj_out2(f)
                    c = self.proj_out(c)
                    c = c.float()
                    f = f.float()
    ...
    def forward(self, x, c, *args, **kwargs):
        ...
        if self.model.conditioning_key is not None:
            ...
            if self.cond_stage_trainable:    
                f, c = self.get_learned_conditioning(c)
                f = self.proj_out2(f)
                c = self.proj_out(c)
        ...
        return self.p_losses(x, torch.cat([c,f], dim=1), t, *args, **kwargs)
        '''
        if self.u_cond_prop<self.u_cond_percent:
            return self.p_losses(x, self.learnable_vector, t, *args, **kwargs)
        else:
            return self.p_losses(x, c+f, t, *args, **kwargs)
        '''
    ...
    def p_losses(self, x_start, cond, t, noise=None, ):
        ...
        if self.first_stage_key == 'inpaint':
            # x_start=x_start[:,:4,:,:]
            mask = 1-x_start[:,8:,:,:]
            #noise = default(noise, lambda: torch.randn_like(x_start[:,:4,:,:]) + 0.1 * mask * torch.randn(x_start[:,:4,:,:].shape[0], x_start[:,:4,:,:].shape[1], 1, 1).to(self.device))
    ...
    def configure_optimizers(self):
        ...
        params = params + list(self.proj_out2.parameters())
        if self.cond_stage_trainable:
            print(f"{self.__class__.__name__}: Also optimizing conditioner params!")
            params = params  + list(self.cond_stage_model.mapper2.parameters()) + list(self.cond_stage_model.final_ln2.parameters())
        if self.learn_logvar:
    ...
    def print_model_parameters(self):
        ...
        print_param(self.model, "UNet + ImageCA")
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None, return_feats: bool = False):
